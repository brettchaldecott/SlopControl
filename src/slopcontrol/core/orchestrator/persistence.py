...
def save(state: OrchestrationState, project_dir: Path) -> Path:
    """Serialise *state* to ``.slopcontrol/orchestration_state.json``."""
    out = project_dir / ".slopcontrol" / _STATE_FILE
...
def load(project_dir: Path) -> OrchestrationState:
    ...
    """
    src = project_dir / ".slopcontrol" / _STATE_FILE
    if not src.exists():
...
def exists(project_dir: Path) -> bool:
    return (project_dir / ".slopcontrol" / _STATE_FILE).exists()
