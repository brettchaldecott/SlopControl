...
def create_gateway_app() -> FastAPI:
    ...
    async def healthz() -> dict:
        ...
        return {"status": "ok", "gateway": "slopcontrol"}
...
async def _stream_response(chain: FallbackChain, messages: list[Any]) -> StreamingResponse:
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        yield f"data: {json.dumps({'id':'slopcontrol-stream-0','object':'chat.completion.chunk','model':'gateway','choices':[{'index':0,'delta':{'role':'assistant'},'finish_reason':None}]})}\n\n"
        yield f"data: {json.dumps({'id':'slopcontrol-stream-0','object':'chat.completion.chunk','model':'gateway','choices':[{'index':0,'delta':{'content':content_str},'finish_reason':None}]})}\n\n"
        yield "data: [DONE]\n\n"
