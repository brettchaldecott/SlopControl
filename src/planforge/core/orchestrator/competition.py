...
class CandidateConfig:
    ...
    agent_name: str          # "planforge", "opencode", etc.
...
class CompetitionManager:
    ...
    def _prepare_workspaces(
        ...
        base = project_dir / ".planforge" / "competition" / f"step_{step_index:03d}"
        if base.exists():
            shutil.rmtree(base)
        ...
        for cfg in candidates:
            ...
            if cfg.workspace_mode == "clone" and project_dir.exists():
                # Shallow copy of project files (skip .planforge, .git, etc.)
    ...
    def _shallow_clone(self, source: Path, dest: Path) -> None:
        ...
        ignore = {".planforge", ".git", ".venv", "__pycache__", ".pytest_cache", "node_modules", ".env"}
    ...
    def _run_internal(
        ...
        """Run an internal domain agent (planforge cad/code/...)."""
